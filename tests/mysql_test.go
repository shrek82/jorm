...
func setupMySQLTestDB(t *testing.T) (*core.DB, func()) {
	t.Helper()
	...
	db, err := core.Open("mysql", dsn, &core.Options{
		MaxOpenConns: 5,
		MaxIdleConns: 5,
	})
	if err != nil {
...
func TestMySQLIntegration(t *testing.T) {
	...
	t.Run("CRUD", func(t *testing.T) {
		...
		if got.Name != "MySQLUser" {
			t.Fatalf("Expected name MySQLUser, got %s", got.Name)
		}

		_, err = db.Model(&User{}).Where("id = ?", id).Update(map[string]any{"age": 31})
		if err != nil {
			t.Fatalf("Update failed: %v", err)
		}

		var updated User
		err = db.Model(&User{}).Where("id = ?", id).First(&updated)
		if err != nil {
		...
		count, err := db.Model(&User{}).Where("id = ?", id).Count()
		if err != nil {
	...
	t.Run("BatchInsertAndQueryHelpers", func(t *testing.T) {
		...
		prefix := fmt.Sprintf("BatchUser_%d_", time.Now().UnixNano())
