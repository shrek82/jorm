...
JORM 验证器通过以下机制确保在高并发场景下的极低开销：

- **零解析开销**：不使用 Struct Tag，规避了运行时反射解析字符串标签的性能损耗。
- **规则预编译**：所有复杂规则（如 `Regexp`, `Datetime`）在 `Rules` 初始化时即完成编译/解析，校验时直接使用结果。
- **高效字段访问**：结合 JORM 的模型缓存，验证器通过字段偏移量（Offset）直接访问内存，性能远高于 `reflect.Value.FieldByName`。
- **并发安全**：`Validator` 和 `Rules` 对象设计为不可变（Immutable），在多协程环境下无需加锁。
